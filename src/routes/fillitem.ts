...
router.get('/', async function (req, res) {
    ...
    let rows: any = await mysqlQuery('select qid,qabstract,qtype,op1,op2,op3,op4,qcas,qcasnum,qcasop from form_question join collection on form_question.title=collection.title where collection.id=?', [req.query.code]);
    for (let i in rows) {
        let item = [];
