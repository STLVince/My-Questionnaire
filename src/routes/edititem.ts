...
router.post('/update', async function(req, res) {
    ...
    for(let i = 0; i < tmp.length; i++) {
        let ret = await mysqlQuery('update form_question set qabstract=?, qtype=?, op1=?, op2=?, op3=?, op4=? where qid=? and title=?', [form.qabstract[i], form.qtype[i], form.op1[i], form.op2[i], form.op3[i], form.op4[i], i + 1, form.formname]);
    }
